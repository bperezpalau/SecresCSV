...
import com.secres.secrescsv.ext.FileDrop;
import com.secres.secrescsv.ext.RowNumberTable;
import com.secres.secrescsv.ext.TableColumnManager;
...
public class View {
    ...
    private JMenu fileMenu, editMenu, viewMenu, helpMenu, separatorMenu, headerMenu, encodingMenu;
    ...
    private JMenuItem aboutMenuItem, separatorMenuItem, separator1MenuItem, separator2MenuItem;
    private JMenuItem separatorNoAskMenuItem,header1MenuItem,header2MenuItem,encoding1MenuItem,encoding2MenuItem,encoding3MenuItem;
    private static JXBusyLabel busyLabel;
    private String separator = ";";
    private boolean noAskSeparator = false;
    private boolean existsHeader = true;
    private boolean askHeader = true;
    private String textFormat = "ANSI";
    ...
    private void createAndShowGUI() {
        ...
        new FileDrop(emptyPanel, new FileDrop.Listener() {
            public void filesDropped(java.io.File[] files) {
                if (!noAskSeparator) {
                    String answer = JOptionPane.showInputDialog(frame, new JPanel(new BorderLayout()), "Change separator (" + separator + ")", JOptionPane.QUESTION_MESSAGE);
                    if (answer != null && !answer.equals("")) {
                        separator = answer;
                    }
                }
                if(askHeader){
                    int answer = JOptionPane.showConfirmDialog(frame, "Exists header in this file", "Header", JOptionPane.YES_NO_OPTION);
    ...
    private void addTablePanel(File path, boolean isLastFile) {
        ...
        Main.saveTextFormat(textFormat);
        Main.saveSeparator(separator);
        Main.existsHeader(existsHeader);
    ...
    private void closeTab(int tabIndex) {
        if (!saveDialog()) {
            int result = JOptionPane.showConfirmDialog(frame, "Not saved.\nDo you want to close tab without save?", "Close", JOptionPane.YES_NO_OPTION);
    ...
    private boolean save() {
        ...
        String answer = (String) JOptionPane.showInputDialog(frame, new JPanel(new BorderLayout()), "Choose the new file name", JOptionPane.QUESTION_MESSAGE, null, null, newPanels.get(tabbedPane.getSelectedComponent()).getName());
        if (answer != null && !answer.equals("")) {
            String oldpathname = newPanels.get(tabbedPane.getSelectedComponent()).getAbsolutePath();
            String newpathname = oldpathname.replace(newPanels.get(tabbedPane.getSelectedComponent()).getName(), answer);

            if (new File(newpathname).exists()) {
                int result = JOptionPane.showConfirmDialog(frame, "Do you want to overwrite the file?", "Overwrite", JOptionPane.YES_NO_OPTION);
                if (result == JOptionPane.YES_OPTION) {
                    overwriteFile = true;
                } else {
                    overwriteFile = false;
                ...
                Main.saveModel(newpathname, ((TablePanel) tabbedPane.getSelectedComponent()).getTable());
                resultSave = true;
            }
            if (overwriteFile) {
                Main.saveModel(newpathname, ((TablePanel) tabbedPane.getSelectedComponent()).getTable());
    ...
    private void createMenuBar() {
        ...
        separatorMenu = new JMenu("Separator");
        separatorMenuItem = new JMenuItem("Change separator");
        separatorMenuItem.setToolTipText("Change separator");
        //JPanel aboutPanel = createSeparatorPanel();
        separatorMenuItem.addActionListener(e -> {
            String answer = JOptionPane.showInputDialog(frame, new JPanel(new BorderLayout()), "Change separator (" + separator + ")", JOptionPane.QUESTION_MESSAGE);
            if (answer != null && !answer.equals("")) {
                separator = answer;
                noAskSeparator = false;
                separator1MenuItem.setSelected(false);
                separator2MenuItem.setSelected(false);
                separatorNoAskMenuItem.setSelected(true);
            }
        });
        ButtonGroup separatorOptions = new ButtonGroup();
        separator1MenuItem = new JRadioButtonMenuItem("Always ;");
        separator1MenuItem.setToolTipText("Always ;");
        separator1MenuItem.setSelected(false);
        separatorOptions.add(separator1MenuItem);
        separator1MenuItem.addActionListener(e -> {
            separator = ";";
            noAskSeparator = true;
        });
        separator2MenuItem = new JRadioButtonMenuItem("Always |");
        separator2MenuItem.setToolTipText("Always |");
        separator2MenuItem.setSelected(false);
        separatorOptions.add(separator2MenuItem);
        separator2MenuItem.addActionListener(e -> {
            separator = "|";
            noAskSeparator = true;
        });
        separatorNoAskMenuItem = new JRadioButtonMenuItem("Dont ask");
        separatorNoAskMenuItem.setToolTipText("Dont ask");
        separatorNoAskMenuItem.setSelected(false);
        separatorOptions.add(separatorNoAskMenuItem);
        separatorNoAskMenuItem.addActionListener(e -> {
            if (separatorNoAskMenuItem.isSelected()) {
                noAskSeparator = true;
            } else {
                noAskSeparator = false;
            }
        });
        separatorMenu.add(separatorMenuItem);
        separatorMenu.add(separator1MenuItem);
        separatorMenu.add(separator2MenuItem);
        separatorMenu.add(separatorNoAskMenuItem);
        
        headerMenu = new JMenu("Header");
        ButtonGroup headerOptions = new ButtonGroup();
        header1MenuItem = new JRadioButtonMenuItem("Ask Header");
        header1MenuItem.setToolTipText("Ask Header");
        header1MenuItem.setSelected(true);
        headerOptions.add(header1MenuItem);
        header1MenuItem.addActionListener(e -> {
            askHeader=true;
        });
        header2MenuItem = new JRadioButtonMenuItem("Never Ask");
        header2MenuItem.setToolTipText("Never Ask");
        header2MenuItem.setSelected(false);
        headerOptions.add(header2MenuItem);
        header2MenuItem.addActionListener(e -> {
            askHeader=false;
        });
        headerMenu.add(header1MenuItem);
        headerMenu.add(header2MenuItem);
        
        encodingMenu = new JMenu("Encoding");
        ButtonGroup encodingOptions = new ButtonGroup();
        encoding1MenuItem = new JRadioButtonMenuItem("ANSI");
        encoding1MenuItem.setToolTipText("ANSI");
        encoding1MenuItem.setSelected(true);
        encodingOptions.add(encoding1MenuItem);
        encoding1MenuItem.addActionListener(e -> {
            textFormat="ANSI";
        });
        encoding2MenuItem = new JRadioButtonMenuItem("UTF-8");
        encoding2MenuItem.setToolTipText("UTF-8");
        encoding2MenuItem.setSelected(false);
        encodingOptions.add(encoding2MenuItem);
        encoding2MenuItem.addActionListener(e -> {
            textFormat="UTF8";
        });
        encoding3MenuItem = new JRadioButtonMenuItem("SYSTEM");
        encoding3MenuItem.setToolTipText("SYSTEM");
        encoding3MenuItem.setSelected(false);
        encodingOptions.add(encoding3MenuItem);
        encoding3MenuItem.addActionListener(e -> {
            textFormat="SYSTEM";
        });
        encodingMenu.add(encoding1MenuItem);
        encodingMenu.add(encoding2MenuItem);
        encodingMenu.add(encoding3MenuItem);
